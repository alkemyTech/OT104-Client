...
test("Send data & get error ", async () => {
  const { getByRole, getByPlaceholderText, container, getByLabelText } = render(
    <MembersForm />
  );

  const nameInput = getByPlaceholderText("Name");
  const fileInput = container.querySelector("#sandbox > input");
  const paraInput = getByLabelText(/descripcion/i);
  ...
  await waitFor(() => {
    expect(screen.getByText("Error to Post")).toBeInTheDocument();
  });
});
